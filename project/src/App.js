...
function App() {
  ...
  useEffect(() => {
    axios.get('http://http://www.missiondreamteam.kro.kr/api/CheckLoginState.php')
    .then(res => {
      if(res.data === false){
        setIsLoggedIn(false);
      } else {
  ...
  const handleAddMission = async () => {
    try {
      ...
      // 새로운 미션 추가
      const res = await axios.post('http://http://www.missiondreamteam.kro.kr/api/Insert_mission.php', {
...
const fetchMissions = async (setMissionList) => {
  try {
    const res = await axios.get(`http://http://www.missiondreamteam.kro.kr/api/Show_mission.php?`)
    if (res.data == null) {
      setMissionList([]);
    } else {
...
const fetchGroups = async (setGroupList) => {
  try {
      const res = await axios.get(`http://http://www.missiondreamteam.kro.kr/api/ShowGroup.php?`)
...
function ToDo(props) {
  ...
  const handleDeleteMission = async (i) => {
    ...
    if (currentHour >= 5 && currentHour < 21) {
      try {
        const res = await axios.post('http://http://www.missiondreamteam.kro.kr/api/Delete_mission.php', {
  ...
  const handleImageUpload = async (e, missionId, index) => {
    ...
    try {
      const res = await axios.post('http://http://www.missiondreamteam.kro.kr/api/MissionImageUpload.php', formData, {
        headers: {
...
function MyCalendar() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const res = await axios.get('http://http://www.missiondreamteam.kro.kr/api/GetPersonalRecord.php');
        if (res.data) {
...
function CreateGroup(props) {
  ...
  useEffect(() => {
    const checkGroupName = async () => {
      if (groupName) {
        try {
          const response = await axios.post('http://http://www.missiondreamteam.kro.kr/api/GroupNameCheck.php', { groupName });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://http://www.missiondreamteam.kro.kr/api/CreateGroup.php', {
...
function JoinGroup(props) {
  const onClickJoin = (event) => {
    ...
    if (!nameIsEmpty && !passwordIsEmpty) {
        ...
        axios.post('http://http://www.missiondreamteam.kro.kr/api/EnterGroup.php',
        {
