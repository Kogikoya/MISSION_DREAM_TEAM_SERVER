...
const UpdateInfoForm = () => {
    ...
    useEffect(() => {
        const fetchUserInfo = async () => {
            try {
                const userRes = await axios.get('http://localhost/api/GetInfo.php');
                const userInfo = userRes.data;
                setUserName(userInfo.name);
                setFormData(prevData => ({ ...prevData, nickName: userInfo.name }));
                const idRes = await axios.get('http://localhost/api/GetId.php');
    ...
    const handleSubmit = async (e) => {
        ...
        if (formIsValid) {
            try {
                ...
                const res = await axios.post('http://localhost/api/UpdateInfo.php', {
    ...
    const handleMemberExit = async () => {
        ...
        if (confirmed) {
            try {
                await axios.post('http://localhost/api/ExitMember.php');
                alert("잘가요");
                // 로그인창으로 이동
                navigate('/login')
            } catch (err) {
